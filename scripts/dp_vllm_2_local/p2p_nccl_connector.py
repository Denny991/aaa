...
class P2pNcclConnector(KVConnectorBase_V1):
        ...
        for request in metadata.requests:
            ...
            for layer_name in forward_context.no_compile_layers:
                ...
                kv_cache = self.p2p_nccl_engine.recv_tensor(
                    self.normalize_tensor_key(request.request_id) + "#" + layer_name,
                    remote_address
                )
        ...
        **kwargs: Any,
        ...
        for request in connector_metadata.requests:
            ...
            self.p2p_nccl_engine.send_tensor(
                self.normalize_tensor_key(request_id) + "#" + layer_name,
    ...
    @staticmethod
    def normalize_tensor_key(request_id: str) -> str:
        """
        Normalize request_id to a shared tensor key.
    
        vLLM wraps the proxy X-Request-Id as:
            cmpl-{X-Request-Id}-0-{local_uuid8}
        P and D each generate a different local_uuid8, so tensor lookup fails.
        Strip the cmpl- prefix and -0-{hexsuffix} to obtain the shared key
        that both sides can agree on.
    
        Example:
            cmpl-___prefill_addr_..._UUID-0-8758f4a0  ->  ___prefill_addr_..._UUID
            cmpl-___prefill_addr_..._UUID-0-b3dae03f  ->  ___prefill_addr_..._UUID
        """
        # cmpl format:     cmpl-{X-Request-Id}-0-{uuid8}   -> {X-Request-Id}
        # chatcmpl format: chatcmpl-{X-Request-Id}-{uuid8}  -> {X-Request-Id}
        normalized = re.sub(r'^(?:chat)?cmpl-(.+?)(?:-0)?-[0-9a-f]{8}$', r'\1', request_id)
        return normalized
